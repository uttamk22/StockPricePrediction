...
def tech_df_api_download(ticker):
    
    try:
    #if ticker is available, pass the ticket and download the weekly data through API alphavantage 
        if ticker:
        ...
        print("Not a valid ticker passed, API is fteching error")
        return None
...
def check_model(x_test, y_test, timeframe, index_date ):
    ...
    if timeframe == 'Week':
        ...
        model_file = 'svr_model_month.joblib'
...
def predict_price(x_real_test, timeframe, test_index_date):
    ...
    st.write('Next ',timeframe, ' Price ' ,df_predict_p)
    return df_predict
...
def run_the_app():
    ...
    if tech_stock_df is not None:
        if timeframe == 'Week':
            x_test, y_test, index_date, x_real_test, test_index_date = create_test_data(tech_stock_df, window, 0)
        else:  
            x_test, y_test, index_date, x_real_test,test_index_date = create_test_data(tech_stock_df, window, 3)
    else:
        if timeframe == 'Week':
